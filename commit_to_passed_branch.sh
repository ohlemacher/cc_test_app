...
function commit_to_branch {
    ...
    sed -i -- 's?fetch = +refs/heads/develop:refs/remotes/origin/develop?fetch = +refs/heads/develop:refs/remotes/origin/*?g' .git/config || die "sed fix failed"
    ...
    git pull  origin passed_unit_tests || die "git pull  passed_unit_tests failed"
